...
### 已实现·活体坐实（本轮）：布线后逐网改宽 = 收敛循环 + 不劣化回退

`apply_track_widths_postroute` 已落地并经 `build_widthpost_det.py` **两相活体 PASS**。核心是
`_converge_net_width(net, target_mil)`——把「逐段 modify 改宽」这条测绘路径**做稳**的收敛原语。
写侧异步态的**四条铁律**(逐条实测彻底定位，非臆测)：

1. **批量 modify 必败**：一发 eval 里循环 `await modify` 多段——首刀 `await modify` 会把**同批余下
   图元重新打回异步态**，第二段起即抛 `t.isAsync is not a function`（连 get-before-modify 同发也
   救不回）。故写必须**一段一发 eval**（`_modify_one`）。
2. **写有额度·会蔓延**：即便一段一发，连续 modify 约 **8~13 段**后剩余图元又整体滑回异步态；此时
   **仅靠通读全板 `get`（settle）已救不回**（实测通读重试 4 次仍败）。唯一确定的复位是
   **`save()`+`reopen_pcb()` 从盘上干净重载**。
3. **reopen 会合并共线同宽段**：重载后 EDA 把同网共线同宽的相邻段**合并**（实测段数 16→10），
   故**不能**靠「一次读定的固定 id 表」逐段写——id 会失效。
4. **极端过宽破连通不可逆**：把段宽推到极端（如 3mm）时物理重叠致合并**丢连接点**→narrow 回去也
   留 **Connection Error**，回退无法复原。故过宽试回退须取**非破坏性**幅度（实测 0.5mm 回退后
   DRC 干净复原；3mm 不可逆）。

**收敛循环**（吸收上述全部）：每轮 `_read_net_line_widths(net)` 读**当前**段 → 挑出未达标者 →
一段一发改至多 `per_round`(=5，<额度上限留裕) 段 → `save+reopen+settle` 复位写额度 → **重读再来**，
直到全网段宽 == target。回退亦是同一原语：「收敛回原宽」（布线网通常单一宽 → 精确复原）。

**留白控制**：freerouting 把线布在**最小间距贴边**（+2mil 即违规），故「适度类宽落地不新增违规」需
先 `set_default_clearance_mm()` 调小板级默认间距让出裕度。活体证：紧凑板上
- 相① PWR→0.5mm：默认间距下挤出 Clearance Error → **收敛回退** → DRC 归 0、线宽还原 [10]mil。
- 相② 调小默认间距后 PWR→0.4mm：**落地生效** → PWR=15.748mil、信号网保持 10mil、DRC 不变。

道法自然·不劣化：改宽若劣化 DRC 则收敛回原样，净效果「要么改进/持平、要么原样」，**永不劣化板子**。
