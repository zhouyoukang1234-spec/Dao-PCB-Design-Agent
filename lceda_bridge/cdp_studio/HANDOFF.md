...
- **文件本源·读写分治(doc_source·本会话逆向)**:`.eprj2` **实为 SQLite**(`SQLite format 3`),但 PCB 几何**不**以扁平 blob 存——`documents/coppers/texts.dataStr` 常空,真实状态由 `branches/history_data/project_history_*` 的**操作日志(op-log)重建**(CRDT 式)。直接逆 op-log **脆弱且随版本漂移**,非本源正道。转锚**官方序列化层**:`sys_FileManager.getDocumentSource(uuid)` 一发取全文(行记录 `{"type":TAG,"ticket":N,"id":..}||{payload}|`),稳定可解析(`doc_source` 原语)。**吃一堑**:对偶的 `setDocumentSource(uuid,src)` **整文回写不生效**(实测恒返 `False`;改 LINE.width 读回不变,仅 DOCHEAD.client 重序列化)——故**读经序列化、写经 typed primitive(`.modify/.create/.delete`)/ 布线经 `importAutoRouteSesFile`**,各得其所,不强行整文写(知止不殆)。`pcb_Document.importChanges(arg)`(argc=1)经探:喂行记录串/对象数组/`{modify:[...]}` 三形**皆返 `true` 却均不生效**(改 LINE.width 读回不变)——其私有 changeset schema 非平凡,**不盲猜硬凑**(返 true 仅证其对未识别输入宽容,非已应用)。结论锚定:**typed primitive 即官方写信道**。
- **op-log 加密已破 + 直写边界·字节级双实证(`eprj2_codec`·本会话)**:把上条「op-log 随版本漂移、不盲逆」从**论断**升级为**实证**。①**破密**:`history_data.dataStr` = `base64(AES-128-GCM(gzip_l1(源记录流)))`;key 取该分支 `project_history_<branch>.key`、iv 取该行 `uuid` 的 hex 前缀(`-N` 后缀截断故同文档各段共 IV)、tag 取末 16B;**加解密 round-trip 实证无损**。`eprj2_codec.py` 落地为 **READ 原语**——可零 GUI 直读任意 `.eprj2` 全量 live 源记录流(明文与 `getDocumentSource` 同构)。②**直写边界(决性反证)**:同一文件(`getCurrentProjectInfo` 确认打开的就是被改文件)、排除一切外部态(`web.db`/IndexedDB/`projects-recovery`/`project_history.snapshot` 实测**均空**)、`pkill`+relaunch 清内存后,字节级改写 op-log 行——改 NET 名 `VCC→DAOPWR`、改纯 PCB 几何 `"Top Layer"→"DAOTOPXX"` **两组实验**:改动**持久留在文件**(重开后 op-log 行仍是新值、**未被编辑器回写覆盖**),但重开后 `getDocumentSource` **仍是原值**(新值不出现)。∴ 编辑器 open 的加载态**并非对落盘 op-log 的重放**,`.eprj2` 文件**不是程控直写的权威面**——这正面**否决**了「直接编写 PCB 文件」作为有效写路径的设想(知止不殆,不强凑)。两方向各归其位:**READ=序列化/op-log 解密(本源可观测全开),WRITE=typed RPC primitive(已建全 14 板 DRC=0)**。
