...
def main():
    ...
    # 内联拉 DRC 逐条违规明细(在已加载上下文):探测 pcb_Drc 上可用的取明细方法,打印前若干条。
    try:
        import dao_eda_cdp_driver as _d
        R = "window._EXTAPI_ROOT_"
        mjs = ("(()=>{try{var o=%s.pcb_Drc;return JSON.stringify("
               "Object.getOwnPropertyNames(Object.getPrototypeOf(o)).concat(Object.keys(o)));"
               "}catch(e){return 'ERR '+e}})()" % R)
        mv, _ = _d.evaluate(f.ws, mjs, await_promise=False, timeout=15)
        rep["drc_methods"] = mv
        detail = None
        for meth in ("getResult", "getResults", "getDrcResult", "getErrorList",
                     "getDrcErrorList", "getAllDrcError", "getViolations", "getErrors"):
            djs = ("(async()=>{try{var r=await %s.pcb_Drc.%s();"
                   "return JSON.stringify(r);}catch(e){return 'ERR '+String(e.message||e).slice(0,60)}})()"
                   % (R, meth))
            dv, _ = _d.evaluate(f.ws, djs, await_promise=True, timeout=60)
            if dv and not dv.startswith('"ERR') and dv not in ("null", "undefined", "[]"):
                detail = {"method": meth, "data": dv[:2000]}
                break
        rep["drc_detail"] = detail
    except Exception as e:
        rep["drc_detail"] = "ERR:" + str(e)[:60]
