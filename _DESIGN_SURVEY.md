...
**2b. 新暴露 — 摆放质量 (placement GAP).**
led_indicator 双层布通后仍剩 1 个 `courtyards_overlap` 错: 这不是布线问题, 是 DNA 把
元件摆得courtyard 相叠 (物理太挤)。dot_matrix 还有 `solder_mask_bridge` (焊盘太近)。
→ 任何布线器都救不了摆放——**下一工具方向: 摆放间距/courtyard 体检与自动拉开**。
