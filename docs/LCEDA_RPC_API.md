...
### 4.5 大板上 `getDsnFile()` 会**瞬态返回 null**（多 IC 大板暴露）
- 在刚放置 + 保存完的**大板**（≳38 元件 / ~100 焊盘）上**立即**取 DSN，`getDsnFile()`
  偶尔返回 `null`（→ 导出 `no file`）——板的异步几何索引尚未落定；小板从不触发。
- 本源解法：`export_dsn` 内建「**保存 + 短歇 + 重试**」——每次重试前再 `save()` 一次推动
  索引落定，直到拿到非空 DSN（默认 4 次 × 1.5s）。这同样适用于其它「取板级产物」的 getter。
...
**确定性**：complex 连续 3 次、mcu 连续 2 次独立运行均 `DRC=0 CLEAN`（数次重试内收敛）——
